...
export const getLessons = async(req, res) => {
    if (req.user.role !== "teacher") return res.status(401).json({ success: false, error: "Unauthorized" });
    try {
        const lessons = await Lesson.find({ teacher: req.user._id }).populate("session", "name").select("-__v");
...
export const getLesson = async(req, res) => {
    if (req.user.role !== "teacher") return res.status(401).json({ success: false, error: "Unauthorized" });
    try {
        const lesson = await Lesson.findById(req.params.id).populate("session", "name").select("-__v");
        if (!lesson) return res.send({ success: false, errorId: 404 });
        res.send({ success: true, lesson });
    } catch (error) {
...
export const getSessions = async(req, res) => {
    if (req.user.role !== "teacher") return res.status(401).json({ success: false, error: "Unauthorized" });
    try {
        const sessions = await Session.find({ teacher: req.user._id }).select("-__v");
...
export const getClasses = async(req, res) => {
    ...
    try {
        const sessions = await Session.find({ teacher: req.user._id }).select("-__v");
        if (!sessions) return res.send({ success: false, errorId: 404 });
        const classes = await Class.find({ session: { $in: sessions } }).select("-__v");
